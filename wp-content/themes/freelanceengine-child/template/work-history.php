...
?>
    <div class="work-history-heading">
        <h4 class="title-big-info-work-history-items">
            <?php 
            if(fre_share_role() ) {
                printf(__("Posted requests (%d)", ET_DOMAIN), $wp_query->found_posts);
            }else{
    ...
    <?php
        ...
        if(have_posts()):
            ...
            echo '<ul style="list-style:none;padding:0;"><li><span class="no-results">'.sprintf(__('%s has not worked on any request yet.', ET_DOMAIN), $display_name ).'</span></li></ul>';
        endif;
        //wp_reset_postdata();
     ?>
