	...
	<?php
		...
		if($project->post_status == 'publish' ){
		 	if( (int) $project->post_author == $user_ID ){ // || $role != FREELANCER
				...
				echo '</div>';
			} else if( $role == 'freelancer' || $role == 'vendor' || !$user_ID ) { ?>
			...
			echo '<div class="col-md-12" >';
			$status = 	array(	'pending' => __('This request is pending', ET_DOMAIN),
								'archive' => __('This request has been archived',ET_DOMAIN) ,
								'reject'  => __('This request has been rejected',ET_DOMAIN) );
