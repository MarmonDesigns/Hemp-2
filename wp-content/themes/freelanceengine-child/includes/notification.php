...
class Fre_Notification extends AE_PostAction
{
    ...
    function bidAccepted($bid_id) {
        ...
        $notification = array(
            ...
            'post_title' => sprintf(__("Bid on request %s was accepted", ET_DOMAIN) , get_the_title($project->ID))
        );
        return $this->insert($notification);
    }
    ...
    function completeProject($project_id, $args) {
        ...
        $notification = array(
            ...
            'post_status' => 'publish',
            'post_title' => sprintf(__("Request %s was completed", ET_DOMAIN) , get_the_title($project->ID))
        );
        return $this->insert($notification);
    }
    ...
    function reviewProjectOwner($project_id, $args) {
        ...
        $notification = array(
            ...
            'post_author' => $project->post_author,
            'post_status' => 'publish',
            'post_title' => sprintf(__("%s reviewed request %s", ET_DOMAIN) , $bidder_name, $project_title)
        );
        return $this->insert($notification);
    }
    ...
    function newMessage($message, $project, $bid) {
        ...
        $notification = array(
            ...
            'post_status' => 'publish',
            'post_type' => $this->post_type,
            'post_title' => sprintf(__("New message on request %s", ET_DOMAIN) , get_the_title($project->ID)) ,
            'post_parent' => $project->ID
        );
        ...
        switch ($type) {
            ...
            case 'new_invite':
                ...
                if(!$project_list) {
                    ...
                    $content.= sprintf(__("%s invited you to join request %s", ET_DOMAIN) , $author , $project );
                }
                break;
    ...
    function notify_sync(){
        ...
        wp_send_json(array(
            'success' => true,
            'data' => $result,
            'msg' => __("Update request successful!", ET_DOMAIN)
        ));
