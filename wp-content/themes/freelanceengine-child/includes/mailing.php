...
Class Fre_Mailing extends AE_Mailing
{
    ...
    function bid_mail($bid_id) {
        ...
        if ($author) {
            ...
            $subject = sprintf(__("Your request posted on %s has a new bid.", ET_DOMAIN) , get_option('blogname'));
    ...
    function review_freelancer_email($project_id) {
        ...
        $subject = __("Request you joined has a review.", ET_DOMAIN);
    ...
    function review_employer_email($project_id) {
        ...
        $subject = __("Your posted request has a review.", ET_DOMAIN);
    ...
    function invite_mail($user_id, $project_id) {
        ...
        if ($user_id && $project_id) {
            ...
            // mail subject
            $subject = sprintf(__("You have a new invitation to join request from %s.", ET_DOMAIN) , get_option('blogname'));
    ...
    function bid_accepted($freelancer_id, $project_id) {
        ...
        // mail subject
        $subject = sprintf(__("Your bid on request %s has been accepted.", ET_DOMAIN) , get_the_title($project_id));
    ...
    function new_report($project_id, $report) {
        ...
        // email subject
        $subject = sprintf(__("Have a new report on request %s.", ET_DOMAIN) , get_the_title($project_id));
    ...
    function close_project($project_id, $message) {
        ...
        // email subject
        $subject = sprintf(__("Request %s was closed.", ET_DOMAIN) , get_the_title($project_id));
    ...
    function quit_project($project_id, $message) {
        ...
        // email subject
        $subject = sprintf(__("User quit your request %s.", ET_DOMAIN) , get_the_title($project_id));
    ...
    function refund($project_id, $bid_accepted) {
        ...
        $subject = sprintf(__("You have got a refund on request %s.", ET_DOMAIN) , get_the_title($project_id));
        ...
        $subject = sprintf(__("Request %s you worked on has refunded.", ET_DOMAIN) , get_the_title($project_id));
        $this->wp_mail($user_email, $subject, $mail_template, array(
            'user_id' => $bid_owner,
            'post' => $project_id
        ));
    ...
    function execute($project_id, $bid_accepted) {
        ...
        // mail to project owner
        $user_email = get_the_author_meta('user_email', $project_owner);
        $subject = sprintf(__("Your presend payment on request %s has been transfered.", ET_DOMAIN) , get_the_title($project_id));
        ...
        // mail to freelancer
        $user_email = get_the_author_meta('user_email', $bid_owner);
        $subject = sprintf(__("You have been sent payment base on request %s.", ET_DOMAIN) , get_the_title($project_id));
        $this->wp_mail($user_email, $subject, $mail_template, array(
            'user_id' => $bid_owner,
            'post' => $project_id
        ));
    ...
    function alert_transfer_money($project_id, $bid_accepted) {
        if (!ae_get_option('manual_transfer')) {
            ...
            // mail to admin
            $subject = sprintf(__("Request %s has been completed and money has transfered.", ET_DOMAIN) , get_the_title($project_id));
            $admin_template = sprintf(__("Request %s has been completed and money has transfered. You can check workspace and request details", ET_DOMAIN) , get_the_title($project_id));
        } else {
            $subject = sprintf(__("Request %s has been completed and waiting your confirm.", ET_DOMAIN) , get_the_title($project_id));
            $admin_template = sprintf(__("Request %s has been completed. Please check it and transfer money to freelancer.", ET_DOMAIN) , get_the_title($project_id));
        }
