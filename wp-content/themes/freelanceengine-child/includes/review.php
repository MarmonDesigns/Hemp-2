...
class Fre_ReviewAction extends AE_Base
{
    ...
    function employer_review_action() {
        ...
        if (!isset($args['project_id'])) {
            wp_send_json(array(
                'success' => false,
                'msg' => __('Invalid request id.', ET_DOMAIN)
            ));
        }
        ...
        */
        if (!$bid_id_accepted) {
            $result = array(
                'succes' => false,
                'msg' => __('Please assign request before complete.', ET_DOMAIN)
            );
            wp_send_json($result);
        }
        ...
        if (!isset($args['score']) || empty($args['score'])) {
            $result = array(
                'succes' => false,
                'msg' => __('You have to rate this profile.', ET_DOMAIN)
            );
            wp_send_json($result);
        }
        ...
        */
        ...
        if (!is_wp_error($comment)) {
            ...
            wp_send_json(array(
                'success' => true,
                'msg' => sprintf(__("You have completed request %s and reviewed %s.", ET_DOMAIN) , $project_title, $freelancer_name)
            ));
        } else {
            wp_send_json(array(
                'success' => false,
    ...
    function freelancer_review_action() {
        ...
        */
        if (!isset($args['score']) || empty($args['score'])) {
            $result = array(
                'succes' => false,
                'msg' => __('You have to rate this request.', ET_DOMAIN)
            );
            wp_send_json($result);
        }
        if (!isset($args['comment_content']) || empty($args['comment_content'])) {
        ...
        */
        if ($status !== 'complete') {
            wp_send_json(array(
                'succes' => false,
                'msg' => __('You can\'t not review this request.', ET_DOMAIN)
            ));
        }
        ...
        if (!empty($comment)) {
            wp_send_json(array(
                'succes' => false,
                'msg' => __('You have reviewed this request.', ET_DOMAIN)
            ));
        }
