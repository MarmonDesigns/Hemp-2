...
class Fre_BidAction extends AE_PostAction
{
    ...
    function bid_sync() {
        ...
        if ($method == 'remove') {
            ...
            if ((int)$accepted == (int)$bid_id) {
                wp_send_json(array(
                    'success' => false,
                    'msg' => __('Error. You have been accepted for this request', ET_DOMAIN)
                ));
            }
        }
    ...
    function bid_accept() {
        ...
        if (!is_wp_error($result)) {
            ...
            wp_send_json(array(
                'success' => true,
                'msg' => __('Request has been assign successful.', ET_DOMAIN)
            ));
        }
    ...
    function fre_check_before_insert_bid($args) {
        ...
        */
        ...
        if (in_array('bid_time', $bid_required_field) && (!isset($args['bid_time']) || empty($args['bid_time'])) ) {
            return new WP_Error('empty_time', __('You have to set the time to finish request.', ET_DOMAIN));
        }
        ...
        if (!$user_ID) return new WP_Error('no_permission', __('Please login to bid a request', ET_DOMAIN));
        
        if (get_post_status($project_id) != 'publish') return new WP_Error('invalid_input', __('This request is not publish.', ET_DOMAIN));
        ...
        if (fre_has_bid($project_id)) return new WP_Error(200, __('You have bid on this request', ET_DOMAIN));
        
        $post_author = (int)get_post_field('post_author', $project_id, 'display');
        
        if ($user_ID == $post_author) {
            return new WP_Error(200, __('You can\'t bid on your request', ET_DOMAIN));
        }
        
        // check role to bid project
        $role = ae_user_role();
        //if (!fre_share_role() && $role != FREELANCER) return new WP_Error(200, __('You have to be a freelancer to bid a request.', ET_DOMAIN));
        ...
        */
        ...
        if (!$profile || !is_numeric($profile_id)) {
            return new WP_Error(200, __('You must complete your profile to bid on a request.', ET_DOMAIN));
        }
    ...
    */
    function fre_update_after_bidding($bid_id) {
        ...
        wp_send_json(array(
            'success' => true,
            'msg' => __('Your bid was successful', ET_DOMAIN)
        ));
    }
    ...
    */
    ...
    function fre_delete_bid($bid_id) {
        ...
        wp_send_json(array(
            'success' => true,
            'msg' => __('Bid has been deleted successfully', ET_DOMAIN)
        ));
    }
    ...
    function assign_project($bid_id) {
        ...
        $result = new WP_Error($code = '200', $message = __('You don\'t have perminsion to accept this request.', ET_DOMAIN) , array());
        ...
        if ($project->post_status != 'publish') {
            ...
            return new WP_Error($code = '200', $message = __('Your request was not pubished. You can not accept a bid!', ET_DOMAIN));
        }
    ...
    */
    function the_title_bid($title, $bid_id = 0) {
        if (is_admin() && is_post_type_archive(BID) && get_post_field('post_type', $bid_id) == 'bid') {
            ...
            // change title
            $title = sprintf(__('%s bid for request "%s"', ET_DOMAIN) , $author, get_the_title($project_id));
        }
        return $title;
    }
    ...
    function manage_bid_column_project($columns) {
        ...
        return array_merge($columns, array(
            'project_title' => __('Request', ET_DOMAIN)
        ));
    }
