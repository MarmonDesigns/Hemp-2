...
class Fre_ReportForm extends AE_Base
{
    ...
    function insert_report($args) {
        ...
        if (!$bid_accepted) {
            return new WP_Error('bid_not_found', __("This request has not accepted any bid.", ET_DOMAIN));
        }
        ...
        if (!current_user_can( 'manage_options' ) && !($bid->post_author == $user_ID || $project->post_author == $user_ID)) {
            return new WP_Error('permission_denied', __("You do not have permission to report a dispute with this request.", ET_DOMAIN));
        }
